...
class ProfilePage extends Component {
            ...
            <Container style={{ maxWidth: "800px" }}>
                ...
                <Row>
                  ...
                  <Col>
                    <div className="profile-info ml-1">
                      <h3>
                        {this.capFirst(this.state.profile.firstname) +
                          " " +
                          this.capFirst(this.state.profile.lastname)}
                      </h3>
                      <span className="mt-5">
                      ...
                      <h6 style={{ color: "#666", marginTop: "0.5em" }}>
              ...
              {this.state.posts.length > 0 ? (
                <PostsList
                  ...
                  newrefresh={this.initialFetcher}
                  posts={this.state.posts}
                  updateIcons={this.state.updateIcons}
                  section={"myposts"}
                />
              ) : (
  ...
  initialFetcher = async () => {
    try {
      ...
      if (response.status === 500) this.props.history.push("/");
      let profile = await response.json();
      console.log(profile);
      response = await fetch(
        "https://appcademix-be.cyclic.app/api/posts/username/" +
          this.props.match.params.username
      );
