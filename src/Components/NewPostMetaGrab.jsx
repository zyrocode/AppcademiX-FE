...
class NewPostMetaGrab extends Component {
  ...
  render() {
    return (
      <>
              ...
              <Row>
                <Col>
                  {!this.state.showImage &&
                    !this.state.link.includes("http" || "www") && (
                ...
                {this.state.showImage && this.state.link && (
                  <Button
                    onClick={() => this.handleMetaTag(this.state.link)}
              ...
              this.state.link.includes("http" || "www") ? (
                ...
                <div className="mt-5">
                  <small className="mt-5">
                    start with http:// to preview url
                  </small>
                </div>
              )}
            </Container>
        ...
        {this.state.openMainForm && (
          <CreatePost data={this.state.urlMetaTags} link={this.state.link} />
        )}
      </>
    );
  }
  ...
  componentDidMount = () => {
    if (!this.props.accessToken) this.props.history.push("/");
  };
  ...
  handleMetaTag = async (url) => {
    try {
      // https://appcademix-be.cyclic.app/api/metatag?url=https://www.youtube.com/watch?v=93p3LxR9xfM
      const response = await fetch(
        `https://appcademix-be.cyclic.app/api/metatag?url=${url}`
      );
      if (response.ok) {
