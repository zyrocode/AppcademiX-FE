    ...
    return (
      <>
        <Navbar>
          <NavbarBrand href="/">
            <RubberBand>
              <img
                width="40px"
                src="https://i.postimg.cc/BnCc3QGK/Appaccademix-Logo-Magenta.png"
              />
            </RubberBand>
          </NavbarBrand>
          ...
          localStorage.getItem("access_token") === "" ? (
            <>
              ...
              <Link to="/register">
                <Button className="ml-2 btn-modal-primary">Register</Button>
              </Link>
            </>
          ) : (
        ...
        {this.state.loginModal && (
          <Login toggle={this.toggleLoginModal} open={this.state.loginModal} />
        )}
      </>
    );
  }
  ...
  componentDidMount = async () => {
    try {
      let responseUsers = await fetch(
        "https://appcademix-be.cyclic.app/api/users"
      );
      let users = await responseUsers.json();
      let responsePosts = await fetch(
        "https://appcademix-be.cyclic.app/api/posts/all"
      );
