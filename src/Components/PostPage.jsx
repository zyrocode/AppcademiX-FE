...
class PostPage extends Component {
    ...
    try {
      if (params) {
        let response = await fetch(
          `https://appcademix-be.cyclic.app/api/posts?sort=${params}&number=1`
        );
    ...
    if (param) {
      try {
        let posts = this.state.posts.filter(
          (post) => post.category == this.capFirst(param)
        );
        console.log(posts);
        this.setState({
          filteredPosts: posts,
  ...
  fetchPosts = async () => {
    try {
      let response = await fetch(
        "https://appcademix-be.cyclic.app/api/posts?sort=ratingsCount"
      );
      let posts = await response.json();
