...
class PostsList extends Component {
  ...
  render() {
    return (
      <Fade>
        <Row>
          {this.state.postIdForDelete && this.state.deleteModalIsOpen && (
            ...
            >
              ...
              <ModalBody>Do you really want to Delete this post?</ModalBody>
              <ModalFooter>
                ...
                <Button
                  color="danger"
                  onClick={() => this.deletePost(this.state.postIdForDelete)}
                >
                  Delete
                </Button>
              </ModalFooter>
                  ...
                  >
                    <Row>
                      ...
                      <Col className="col-6 post-body">
                        ...
                        <Row>
                          ...
                          {this.props.updateIcons && (
                            <>
                              ...
                              >
                                <span>
                                  <FontAwesome className="mr-1" name="trash" />
                                  Delete
                                </span>
                              </div>
                              &nbsp; &nbsp;
                              <Link to={"/editpost/" + post._id}>
                                ...
                                >
                                  <span>
                                    <FontAwesome className="mr-1" name="edit" />
                                    Edit
                                  </span>
                                </div>
                              </Link>
                        ...
                        <Row>
                          <h6
                            style={{ fontSize: "medium", paddingTop: "0.5em" }}
                          >
                            Posted By{" "}
                            <Link
                      ...
                      <Col className="p-0 post-rate">
                        ...
                        post.ratings.find(
                          ...
                          <span
                            onClick={(e) => this.ratePost(post, e)}
                            className="rate2"
                          >
                            <FontAwesome
                              className="rate2-color"
                              name="star"
                              size="2x"
                            />
        ...
        {!this.props.userInfo.accessToken && this.state.openLogin && (
          <Login
            toggle={() => this.setState({ openLogin: !this.state.openLogin })}
  ...
  componentDidUpdate = (prevProps, prevStates) => {
    if (
      ...
      prevStates.posts.length !== this.state.posts.length
    ) {
      this.filterPosts(this.props.posts);
  ...
  checkUserRate = (post) => {
    let result = post.ratings.filter(
      (post) => post.upvotedBy === this.props.userInfo.username
    );
    console.log(result);
  };
  ...
  ratePost = async (post, e) => {
    ...
    if (this.props.accessToken && this.props.userInfo.username) {
      ...
      let ratingsFind = post.ratings.find(
        ({ upvotedBy }) => upvotedBy === this.props.userInfo.username
      );
      ...
      if (ratingsFind !== undefined) {
        try {
          ...
          this.props.posts.forEach((singlePost, index) => {
            if (singlePost._id == post._id) indexOfThePost = index;
          });
          ...
          await fetch(
            `https://appcademix-be.cyclic.app/api/ratings/${post._id}/${this.props.userInfo.username}`,
            {
              method: "DELETE",
              headers: {
  ...
  filterPosts = (postsList) => {
    switch (this.props.section) {
      case "today":
        {
          let posts = postsList.filter(
            (post) => post.createdAt.substring(0, 10) === this.state.today
          );
          this.setState({
            posts: posts,
            title: "Today",
      ...
      case "yesterday":
        {
          let posts = postsList.filter(
            (post) => post.createdAt.substring(0, 10) === this.state.yesterday
          );
          this.setState({
            posts: posts,
            title: "Yesterday",
  ...
  deletePost = async (id) => {
    ...
    try {
      const resp = await fetch(
        `https://appcademix-be.cyclic.app/api/posts/${this.props.userInfo.username}/${id}/`,
        {
          method: "DELETE",
          headers: {
            Authorization: "Bearer " + this.props.accessToken,
