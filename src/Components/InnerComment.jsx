...
class InnerComponent extends Component {
    ...
    return (
      <>
        {this.state.commentIdForDelete && this.state.deleteReplyModalIsOpen && (
          <Modal
            isOpen={this.state.deleteReplyModalIsOpen}
            toggle={this.toggleDelete}
          >
            <ModalHeader toggle={this.toggleReplyDelete}></ModalHeader>
            <ModalBody>Do you really want to Delete this Reply?</ModalBody>
            <ModalFooter>
              <Button color="primary" onClick={this.toggleReplyDelete}>
                Cancel
              </Button>
              <Button color="danger" onClick={this.deleteReplyComment}>
                Delete
              </Button>
            </ModalFooter>
          </Modal>
            ...
            <Comment.Group key={index}>
              <Comment>
                ...
                <Comment.Content>
                  ...
                  {this.state.openReplyBoxForEdit ? null : (
  ...
  editReplyComment = async (e) => {
    ...
    try {
      ...
      let response = await fetch(
        `https://appcademix-be.cyclic.app/api/reply/${commentId}/${this.state.replyId}`,
        {
          method: "PUT",
          headers: {
  ...
  deleteReplyComment = async () => {
    try {
      ...
      let response = await fetch(
        `https://appcademix-be.cyclic.app/api/reply/${commentIdForDelete}/${replyIdForDelete}`,
        {
          method: "DELETE",
          headers: {
            Authorization: "Bearer " + this.props.accessToken,
