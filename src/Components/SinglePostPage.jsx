...
class SinglePostPage extends Component {
              ...
              <Row>
                <Col>
                  <Row>
                    ...
                    <Col>
                      <h4 className="m-2">
                        {this.state.post.title.toUpperCase()}
                      </h4>
                ...
                <Col className="col-3">
                  <Row>
                    <Col>
                      {this.state.post.ratings.length > 0 &&
                      this.state.post.ratings.find(
                        ({ upvotedBy }) =>
                          upvotedBy === this.props.userInfo.username
                      ) ? (
                        <span
                          onClick={() => this.ratePost(this.state.post)}
                          className="rate2 mr-0"
                        >
                          <FontAwesome name="star" size="2x" />
                          <span className="rate-number">
                            {this.state.post.ratingsCount}
                          </span>
                        </span>
                      ) : (
                        <span
                ...
                <Col className="mt-2 ml-4 ">
                  ...
                  <Row>
                    <h6 style={{ fontStyle: "italic", fontSize: "small" }}>
                      <Moment fromNow>{this.state.post.createdAt}</Moment>
                    </h6>
                  </Row>
                </Col>
                <Col className="m-2 ">
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    if (
      prevState.openForEdit !== this.state.openForEdit ||
  ...
  getAllComments = async () => {
    try {
      let response = await fetch(
        `https://appcademix-be.cyclic.app/api/comments/${this.state.post._id}?sort=updatedAt`
      );
  ...
  fetchPost = async () => {
    try {
      console.log("HEREEE");
      let response = await fetch(
        "https://appcademix-be.cyclic.app/api/posts/" +
  ...
  ratePost = async (post, e) => {
    ...
    if (this.props.accessToken && this.props.userInfo.username) {
      let ratingsFind = this.state.post.ratings.find(
