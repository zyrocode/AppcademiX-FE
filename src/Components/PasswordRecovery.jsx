        ...
        <Row>
          {this.state.passModalIsOpen && (
            ...
            >
                    ...
                    <Col>
                      {this.state.secondModalOpen ? (
                        <>
                          ...
                          <Container>
                            <p>
                              Check your inbox. We've sent a link to reset your
                              password to:{" "}
                              <span style={{ color: "#EF3B59" }}>
                                {" "}
                                {this.state.email}
                              </span>
                              .
                            ...
                            <p>
                              If you don't recieve this email in few minutes,
                              please make sure it was not sent to your spam
                              folder.
                            </p>
                          </Container>{" "}
                        </>
              ...
              {!this.state.secondModalOpen && (
                <ModalFooter>
                  <Button color="primary" onClick={this.cancelModal}>
                    Cancel
                  </Button>
                  <Button
                    color="danger"
                    onClick={() => this.sendPassReq(this.state.email)}
                  >
                    Reset Your Password
                  </Button>
                </ModalFooter>
                ...
                <Form onSubmit={this.submitPost}>
                  ...
                  <FormGroup>
                    ...
                    <Input
                      ...
                      required
                      style={{ borderColor: "#EF3B59", borderRadius: "3em" }}
                    ></Input>
                  </FormGroup>
                  <FormGroup>
                  ...
                  <div className="mx-auto text-center m-5">
                    <Button className="btn-modal-primary rounded-pill w-50">
                      Reset Password
                    </Button>
                  </div>
                </Form>
  ...
  sendPassReq = async (emailForPassReset) => {
    try {
      ...
      let response = await fetch(
        "https://appcademix-be.cyclic.app/api/auth/forgotpwd",
        {
          method: "POST",
          headers: {
  ...
  submitPost = async (e) => {
    ...
    try {
      if (this.state.password === this.state.confirmPassword) {
        ...
        let response = await fetch(
          "https://appcademix-be.cyclic.app/api/auth/resetpassword",
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          toast.success(credentials.message);
          this.props.history.push("/");
        } else toast.error("oops something went wrong, try again later");
        ...
        toast.error(
          "The Confirm Password does not match the password you want to change"
        );
      }
    } catch (ex) {}
  };
